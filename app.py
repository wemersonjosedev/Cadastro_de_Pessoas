...
UPLOAD_FOLDER = os.path.join('static', 'uploads', 'fotos')  # Caminho corrigido
...
def dashboard():
    ...
    """
    total_cadastros = Usuario.query.count()
    cadastros_hoje = Usuario.query.filter(db.func.date(Usuario.datacadastro) == datetime.today().date()).count()
    cadastros_mes = Usuario.query.filter(db.func.month(Usuario.datacadastro) == datetime.today().month,
                                         db.func.year(Usuario.datacadastro) == datetime.today().year).count()
    cadastros_ano = Usuario.query.filter(db.func.year(Usuario.datacadastro) == datetime.today().year).count()
